...
//PAGE DE FONCTIONS DELETE_ALL SQL
	...
	//SUPPRIMER COMPTE
	function delete_account($bdd, $ID)
	{
		//supprime commentaires
		delete_all_coms($bdd, $ID);
		//supprime publications
		delete_all_publi($bdd, $ID);
		//supprime messages
		delete_all_messages($bdd, $ID);
		//supprime evenements
		delete_all_events($bdd, $ID);
		//supprime groupes
		delete_all_groups($bdd, $ID);
		//supprime contacts
		delete_all_contacts($bdd, $ID);
		//supprime l'utilisateur
		delete_user($bdd, $ID);
	}
?>