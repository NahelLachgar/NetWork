...
CREATE TABLE IF NOT EXISTS `NetWork`.`coms` (
  ...
  PRIMARY KEY (`id`, `user`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) ,
  INDEX `fk_coms_users1_idx` (`user` ASC) ,
  CONSTRAINT `fk_coms_users1`
    FOREIGN KEY (`user`)
    REFERENCES `NetWork`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `NetWork`.`notifications`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `NetWork`.`notifications` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user` INT NOT NULL,
  `contact` INT NOT NULL,
  `content` VARCHAR(255) NOT NULL,
  ...
  `type` ENUM('message', 'contactAdd', 'groupAdd', 'groupMessage', 'eventAdd', 'contactRefuse', 'contactAccept', 'contactDelete') NOT NULL,
