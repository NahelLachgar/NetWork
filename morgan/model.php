...
function addUser($lastName, $firstName, $email, $phone, $photo, $password, $status, $job, $company, $town) {
    ...
    function getSearch($sid,$name)
    {
        $db =  dbConnect();
        $res  = "%".trim($name)."%" ;
        $req =  $db->prepare('SELECT users.id as idContact,users.lastname,users.name,users.email,users.phone,users.job,users.company,users.town,users.status FROM users WHERE users.id != ? AND (users.lastname LIKE ?  OR users.name LIKE ?) ');
